...
def test_existing_dir():
    ...
    plotter = signal_plot(parameters, labels, 'rmse',
    ...
    with pytest.raises(TypeError):
        plotter = signal_plot(parameters, labels, 'rmse',
            predictor, 'T_release')
    with pytest.raises(TypeError):
        plotter = signal_plot(1, labels, 'rmse',
            predictor, 'T_release/')
    with pytest.raises(TypeError):
        plotter = signal_plot(parameters, labels, 'banana',
            predictor, 'T_release/')
    with pytest.raises(TypeError):
        plotter = signal_plot(parameters, 6, 'rmse',
            predictor, 'T_release/')
    with pytest.raises(TypeError):
        plotter = signal_plot(parameters, labels, 'rmse',
            'foobar', 'T_release/')
    ...
    plotter = signal_plot(parameters, labels, loss_func,
        predictor, 'T_release/')
